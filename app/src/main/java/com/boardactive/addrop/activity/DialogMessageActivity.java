...
import com.boardactive.addrop.model.NotifType;
...
public class DialogMessageActivity extends AppCompatActivity {
    ...
    private void initComponent() {
        String dateCreated = new SimpleDateFormat("MM/dd/yyyy").format(new Date(messageEntity.dateCreated));
        ...
        if (messageEntity.isTestMessage.equals("1") ){
        ...
        if (from_notif) {
            ...
            if (MainActivity.active && (type.equalsIgnoreCase(NotifType.NORMAL.name()) || type.equalsIgnoreCase(NotifType.IMAGE.name()))) {
        ...
        (findViewById(R.id.bt_delete)).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!from_notif && position != -1) {
                    dao.deleteMessage(messageEntity.getId());
